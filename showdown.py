...
class Showdown:
    ...
    def _update_timer(self):
        ...
        if self.timer <= 0:
            self.txt = self.font.render('Game Over', True, (0,0,0), (230,230,230))
            self.screen.blit(self.txt, (450,335))
            self.game_active = False
            pygame.mixer.music.stop()
            if self.racecar.lapcount > 0 and self.continue_allowed == True:
                self.txt = self.font.render('Continue?', True, (0,0,0), (230,230,230))
                self.screen.blit(self.txt, (450,335))
        else:
            self.txt = self.font.render('Timer: ' +str(round(self.timer, 2)), True, (0,0,0), (230,230,230))
            self.screen.blit(self.txt, (450,335))
    ...
    def _check_keyup_events(self, event):
        ...
        if event.key == pygame.K_w:
            #Code to decrease speed gradually here
            self.racecar.is_moving = False
            #print("YOU STOPPED HITTING W") #DEBUG LINE
