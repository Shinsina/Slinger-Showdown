...
class Racecar(Sprite):
    ...
    def __init__(self, sd_game):
        ...
        self.carpoints = [(550, 530), (850, 515), (1055, 282),
                          (850, 49), (550, 34), (250, 49), (45, 282), (250, 515)]
        self.ptp = [(300, -15), (205, -233), (-205, -233), (-300, -15),
                    (-300, 15), (-205, 233), (205, 233), (300, 15)]
    ...
    def update(self):
        ...
        if abs(self.carpoints[self.index][0]-abs(self.rect.centerx)) - abs(self.ptp[self.index][0]) == 0 or abs(self.carpoints[self.index][1]-abs(self.rect.centery)) - abs(self.ptp[self.index][1]) == 0:
            # If the x-value when checkpoint is hit is not what it's supposed to be
            if(abs(self.carpoints[self.index][0]-abs(self.rect.centerx)) - abs(self.ptp[self.index][0]) != 0):
                # Set the position to the proper value
                self.x = self.carpoints[self.index+1][0]
            # If the y-value when checkpoint is hit is not what it's supposed to be
            elif((abs(self.carpoints[self.index][1]-abs(self.rect.centery)) - abs(self.ptp[self.index][1])) != 0):
                # Set the position to the proper value
                self.y = self.carpoints[self.index+1][1]
            if(self.index == 7):  # This is for the last checkpoint
                self.index = 0  # Reset counter
                self.movement = self.ptp[self.index]
                # Make a vector to use for vector calculation later
                self.movement = Vector2(self.movement[0], self.movement[1])
                self.distance = self.movement.length()  # Set distance to how far you moved
                # Round distance to an int and add it to total distance
                self.distancetotal += int(self.distance)
                ...
                self.distancetotal += int(self.distance)
                self.movement.normalize_ip()  # Create a normal vector for the movement
                # Calculate the angle to the next checkpoint in order to travel to it
                self.angle = - \
